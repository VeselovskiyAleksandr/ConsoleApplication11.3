...
int main()
{
	...
	float power, health, resistanceIndex, damage;
	...
	if (health > 0 && health < 1 && resistanceIndex > 0 && resistanceIndex < 1) {
		do {
			...
			damage = pow(resistanceIndex,power);
			cout << "\nВ результате выстрела Орку нанесён урон здоровью " << damage;
			cout << "\nУ Орка осталось " << health * damage << " очков здоровья";
			health *= damage;
			if (health < 0.1) {
				cout << "\n\n                Орк погиб!";
			}
